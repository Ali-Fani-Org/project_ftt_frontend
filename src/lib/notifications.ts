...
class TauriNotificationService implements NotificationService {
	...
	constructor() {
		...
		if (!browser) {
			logger.warn(
				`[NotificationService ${this.serviceId}] Can only be used in browser environment`
			);
	...
	async connect(): Promise<void> {
		...
		try {
			...
			if (error.name !== 'AbortError') {
				...
				logger.error(
					`[NotificationService ${this.serviceId}] ❌ Failed to start long polling:`,
					error
				);
			}
			...
			try {
				...
				if (!response.ok) {
					if (response.status === 401) {
						logger.error(
							`[NotificationService ${this.serviceId}] 🚫 401 Unauthorized - clearing invalid auth token`
						);
						...
						try {
							...
							logger.error(
								`[NotificationService ${this.serviceId}] ❌ Failed to clear auth token:`,
								error
							);
						}
				...
				if (data.has_new_notifications && data.notifications.length > 0) {
					for (const notification of data.notifications) {
						logger.debug(
							`[NotificationService ${this.serviceId}] 🔔 New notification:`,
							notification
						);
	...
	private async showNotification(notification: NotificationData): Promise<void> {
		logger.debug(
			`[NotificationService ${this.serviceId}] 🎯 showNotification() called for:`,
			notification
		);
		try {
			...
			if (!invoke) {
				logger.warn(
					`[NotificationService ${this.serviceId}] ⚠️ Tauri API not available for notifications`
				);
				return;
			}

			logger.debug(
				`[NotificationService ${this.serviceId}] ✅ Tauri API available, preparing notification`
			);
			...
			logger.debug(
				`[NotificationService ${this.serviceId}] ✅ Native notification requested: ${fullTitle} - ${notification.message}`
			);
		} catch (error) {
			logger.error(
				`[NotificationService ${this.serviceId}] ❌ Failed to show native notification:`,
				error
			);
		}
	}
	...
	private async acknowledgeNotification(notificationId: string): Promise<void> {
		logger.debug(
			`[NotificationService ${this.serviceId}] 📤 acknowledgeNotification() called for ID:`,
			notificationId
		);
		try {
			const token = get(authToken);
			...
			if (!token) {
				logger.warn(
					`[NotificationService ${this.serviceId}] ⚠️ No auth token available for notification acknowledgment`
				);
				return;
			}
			...
			if (response.ok) {
				logger.debug(
					`[NotificationService ${this.serviceId}] ✅ Notification ${notificationId} acknowledged`
				);
			} else {
				// Handle authentication failures for acknowledgment
				if (response.status === 401) {
					logger.error(
						`[NotificationService ${this.serviceId}] 🚫 401 Unauthorized during acknowledgment - clearing invalid auth token`
					);
					...
					try {
						authToken.set(null);
						logger.debug(
							`[NotificationService ${this.serviceId}] 🗑️ Invalid auth token cleared during acknowledgment`
						);
					} catch (error) {
						logger.error(
							`[NotificationService ${this.serviceId}] ❌ Failed to clear auth token:`,
				...
				logger.warn(
					`[NotificationService ${this.serviceId}] ⚠️ Failed to acknowledge notification ${notificationId}:`,
					response.status
				);
			}
		} catch (error) {
			logger.error(
				`[NotificationService ${this.serviceId}] ❌ Failed to acknowledge notification:`,
				error
			);
		}
	}
	...
	private getNotificationTitle(type: NotificationData['type']): string {
		...
		logger.debug(
			`[NotificationService ${this.serviceId}] 📝 Mapped type "${type}" to title "${title}"`
		);
		return title;
	}
	...
	private handleReconnect(): void {
		logger.debug(
			`[NotificationService ${this.serviceId}] 🔄 handleReconnect() called, attempts: ${this.reconnectAttempts}/${this.maxReconnectAttempts}`
		);
		this.connected = false;
		this.isConnecting = false;
		this.isPolling = false;
		...
		if (!token) {
			logger.debug(
				`[NotificationService ${this.serviceId}] 🛑 Reconnection blocked - no auth token available`
			);
			this.lastError = 'Authentication required - please log in first';
			return;
		}
		...
		if (this.reconnectAttempts < this.maxReconnectAttempts) {
			...
			setTimeout(() => {
				logger.debug(
					`[NotificationService ${this.serviceId}] 🔄 Triggering reconnection attempt ${this.reconnectAttempts}`
				);
				this.connect().catch(logger.error);
			}, delay);
		} else {
			logger.error(
				`[NotificationService ${this.serviceId}] 🚫 Max reconnection attempts reached. Giving up.`
			);
