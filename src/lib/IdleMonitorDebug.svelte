			...
			<div class="alert alert-warning">
				...
				<div>
					...
					<div class="text-sm mt-1">
						<button class="btn btn-xs btn-outline" onclick={loadFeatureFlags}
							>Reload feature flags</button
						>
					</div>
				</div>
			</div>
		...
		{:else}
			...
			<div class="stats stats-vertical lg:stats-horizontal shadow mb-6">
				...
				<div class="stat">
					...
					<div class="stat-desc">Current idle duration</div>
				</div>
				...
				<div class="stat">
					...
					<div class="stat-desc">Since last activity</div>
				</div>
			</div>
			...
			<div class="alert alert-success mb-4">
				...
				<div>
					...
					<div class="text-sm">
						Backend is successfully detecting idle/active states and emitting events.
					</div>
					{#if activityLogs.length === 0}
						<div class="text-sm text-warning mt-1">
							⚠️ No frontend logs yet - checking event listeners...
						</div>
					{/if}
				</div>
			</div>
				...
				{:else}
					...
					{#if activityLogs.length > 20}
						<p class="text-xs text-base-content/50 mt-2">
							Showing latest 20 of {activityLogs.length} logs
						</p>
					{/if}
				{/if}
			</div>
			...
			<div class="alert alert-success">
				...
				<div>
					...
					<p class="text-sm">
						Activity logs are stored locally and ready to be synchronized with the server when
						needed.
					</p>
				</div>
			</div>
