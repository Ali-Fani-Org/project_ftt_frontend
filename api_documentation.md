...
```json
{
    ...
    "duration": "string (total seconds as string, e.g., '8100.0' for 2h 15m)",
    "is_active": "boolean",
...
```json
{
    ...
    "project": 1,
    "tags": [1, 2],
    "start_time": "2025-11-29T16:30:00.000Z"
}
```

**Field Descriptions:**
- `title` (required): Title of the time entry
- `description` (optional): Description of the task
- `project` (required): ID of the project (must be a member)
- `tags` (optional): Array of tag IDs (must belong to the user)
- `start_time` (optional): Custom start time in ISO 8601 format. If not provided, current time is used.
...
**Response (400 Bad Request) - Active Entry Exists:**
```json
{
    "non_field_errors": ["You already have an active time entry. Please stop it first before creating a new one."]
}
```

**Response (400 Bad Request) - Start Time Validation:**
```json
{
...
### Update Time Entry Title, Description, and Tags
**Endpoint:** `PATCH /api/time_entries/{id}/`
**Purpose:** Update the title, description, and/or tags of an existing time entry

**Path Parameters:**
- `id` (integer): The ID of the time entry to update
...
**Field Descriptions:**
- `title` (optional): New title for the time entry (max 256 characters)
- `description` (optional): New description (can be blank or null)
- `tags` (optional): Array of tag IDs to replace existing tags (must belong to the user)
